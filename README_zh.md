...
- 🔍 **全面的颜色检测** — 支持 Hex（`#RGB`、`#RRGGBB`、`#RGBA`、`#RRGGBBAA`）、`rgb()`/`rgba()`、`hsl()`/`hsla()` 以及 **148 种 CSS 命名颜色**
- 🟦 **内联色块预览** — 在每个检测到的颜色值旁插入 14×14 px 圆角色块
- 🌓 **暗黑模式兼容** — 边框颜色根据色块亮度自动调整（亮色块用深色边框，暗色块用浅色边框）
- 🏁 **透明度展示** — 半透明颜色下方显示棋盘格图案，直观体现透明度
...
- 🛡️ **安全不干扰** — 自动跳过 `<script>`、`<style>`、`<input>`、`<textarea>` 和 `contenteditable` 区域
...
| RGB（空格语法） | `rgb(255 0 0)`、`rgb(255 0 0 / 0.5)` |
| HSL（逗号语法） | `hsl(200, 80%, 60%)` |
| HSLA（逗号语法） | `hsla(200, 80%, 60%, 0.7)` |
| HSL（空格语法） | `hsl(200 80% 60%)`、`hsl(200 80% 60% / 0.7)` |
| 命名颜色 | `red`、`blue`、`rebeccapurple`、`transparent` …（共 148 种） |
